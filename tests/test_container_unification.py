...
def test_compose_uses_single_unified_service():
    ...
    assert f'- MT3_SERVICE_URL=${{MT3_SERVICE_URL:-{mt3_config.DEFAULT_MT3_SERVICE_URL}}}' in text
    assert f'- MT3_MODEL=${{MT3_MODEL:-{mt3_config.DEFAULT_MT3_MODEL}}}' in text
    assert '- MT3_DEVICE=${MT3_DEVICE:-auto}' in text
    assert f'- MT3_TIMEOUT=${{MT3_TIMEOUT:-{mt3_config.DEFAULT_MT3_TIMEOUT}}}' in text
    assert f'- MT3_CHECKPOINT_ROOT=${{MT3_CHECKPOINT_ROOT:-{mt3_config.DEFAULT_MT3_CHECKPOINT_ROOT}}}' in text
    assert f'- MT3_CACHE_DIR=${{MT3_CACHE_DIR:-{mt3_config.DEFAULT_MT3_CACHE_DIR}}}' in text
    assert f'- MT3_OUTPUT_PATH=${{MT3_OUTPUT_PATH:-{mt3_config.DEFAULT_MT3_OUTPUT_PATH}}}' in text
